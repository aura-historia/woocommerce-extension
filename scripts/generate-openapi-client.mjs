...
function forEachGeneratedPhpFile(config, callback) {
    const outputPath = path.join(projectRoot, config.outputPath);
    ...
    while (stack.length > 0) {
        ...
        for (const entry of readdirSync(currentPath, { withFileTypes: true })) {
            ...
            callback(entryPath, path.relative(outputPath, entryPath).replace(/\\/g, '/'));
        }
    }
}

function prependPhpcsDisable(content, sniffs) {
    if (!Array.isArray(sniffs) || sniffs.length === 0) {
        return content;
    }

    const directive = `// phpcs:disable ${sniffs.join(', ')}`;
    ...
    if (!content.includes(marker)) {
        throw new Error(`Could not apply PHPCS disable directive to generated client file.`);
    }

    return content.replace(marker, `${marker}${directive}\n\n`);
}

function applyGeneratedPhpcsDirectives(config) {
    forEachGeneratedPhpFile(config, (entryPath, relativePath) => {
        let content = readFileSync(entryPath, 'utf8');
        const sniffs = ['WordPress.Security.EscapeOutput.ExceptionNotEscaped'];
...
async function main() {
    ...
    applyGeneratedPhpcsDirectives(config);
